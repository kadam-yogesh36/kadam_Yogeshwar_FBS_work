...
class BankController {
	...
	public void createAccount() {
		...
		if (type == 1)
			acc = new SavingAccount(name, bal, 1000, 5.8);
		else if (type == 2)
			acc = new SalaryAccount(name, bal, "Company");
		else if (type == 3)
			acc = new CurrentAccount(name, bal, 5000);
		else {
	...
	public void applyLoan() {
		...
		LoanAccount loan = new LoanAccount(name, 0, // opening balance
				creditScore, emi, loanAmount, interest);
				...
				for (int j = 1; j < txnCount; j++) { // skip index 0 (opening balance)
					...
					if (txns[j].date.toLocalDate().equals(LocalDate.now())) {
						...
						if (type.equals("DEPOSIT"))
							totalDeposits += txns[j].amount;
						else if (type.equals("WITHDRAW"))
							totalWithdrawals += txns[j].amount;
						else if (type.contains("EMI"))
							totalEMI += txns[j].amount;
					}
				}
			}
		}
